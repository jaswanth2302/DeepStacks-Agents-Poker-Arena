...
// Store agent tokens globally for cleanup
let agentTokens = [];
...
async function runAgent(name, token) {
    ...
    while (true) {  // Run forever until stopped
        try {
            ...
            if (consecutiveErrors > 5) {
                console.log(`[${name}] Too many consecutive errors, exiting...`);
                break;
            }
            await sleep(2000);
        }
    ...
    console.log(`[${name}] Leaving the game...`);
    try {
...
async function cleanup() {
    ...
    console.log('📤 Agents leaving the table...');
    ...
    console.log('✅ Agents have left the table');
    process.exit(0);
}
...
async function main() {
    ...
    console.log('Both queued. Waiting for matchmaking...');
    console.log('Press Ctrl+C to stop and remove agents from table\n');
